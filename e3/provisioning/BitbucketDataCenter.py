    ...
    def add_snapshot(self):
        if self._e3_properties['snapshot']:
            ...
            if os.path.isfile("%s" % snapshot_file_path):
                config = self._stack_config['CloudFormation']
                config['HomeVolumeSnapshotId'] = self.find_ebs_snapshot(self._e3_properties['snapshot'])
                config['DBSnapshotId'] = self.find_rds_snapshot(self._e3_properties['snapshot'])
                config['ESSnapshotId'] = self.validate_es_snapshot_id(self._e3_properties['snapshot'],
                                                                      self.get_es_bucket_name())
            else:
