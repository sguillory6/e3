...
class HazelcastOperationServiceJmx(Graph):
    _graph_config = {
        'file': '%s-hazelcast-operations.png',
        'title': 'Hazelcast Operations on %s',
        'label': 'Operations',
        'stack': False,
        'config': [
            {
                'gauge-remoteOperationCount': [
                    {
                        'ds': 'value',
                        'multi': 1,
                        'desc': 'Count',
    ...
    def render(self, node_name, start, end, graph_width=350, graph_height=100):
        ...
        data_dir = os.path.join(self._server_dir,
                                'GenericJMX-com.hazelcast_HazelcastInstance.OperationService.hazelcast.oper')
